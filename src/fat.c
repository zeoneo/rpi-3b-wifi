...
struct __attribute__((packed, aligned(2))) partition_info {
    ...
    uint8_t type;    // partition type (01h = 12bit FAT, 04h = 16bit FAT, 05h = Ex MSDOS, 06h = 16bit FAT (>32Mb), 0Bh =
                     // 32bit FAT (<2048GB))
...
uint32_t search_directory_cluster_contents(uint32_t directory_first_sector, uint8_t* next_dir_name,
                                           uint32_t next_index);
...
bool initialize_fat(read_handler_t read_handler) {
    ...
    if ((bpb->fat16_table_size == 0) && (bpb->root_entry_count == 0)) {
        ...
        current_sd_partition.firstDataSector = bpb->reserved_sector_count + bpb->hidden_sector_count +
                                               (bpb->fat_type_data.ex_fat32.fat_table_size_32 * bpb->fat_table_count);
        current_sd_partition.dataSectors = bpb->fat32_total_sectors - bpb->reserved_sector_count -
                                           (bpb->fat_type_data.ex_fat32.fat_table_size_32 * bpb->fat_table_count);
        ...
        current_sd_partition.firstDataSector =
            current_sd_partition.unusedSectors + (bpb->fat_table_count * bpb->fat16_table_size) + 1;
        // data sectors x sectorsize = capacity ... I have check this on PC and gives right calc
        current_sd_partition.dataSectors = bpb->fat32_total_sectors - (bpb->fat_table_count * bpb->fat16_table_size) -
                                           33; // -1 see above +1 and 32 fixed sectors
    ...
    printf("Reserved sectors :%lu Fat size : %d bytesPerSector: %d \n", current_sd_partition.reservedSectorCount,
           current_sd_partition.fatSize, current_sd_partition.bytesPerSector);
    return true;
}
...
uint32_t search_entries_in_sector(uint32_t sector_number, uint8_t* buffer, uint8_t* next_dir_name,
                                  uint32_t next_index) {
    uint32_t limit = 512;
    uint32_t index = 0;
                ...
                if (lfn_entry->LDIR_SeqNum == 0x1 || lfn_entry->LDIR_SeqNum == 0x41) {
                    ...
                    if (is_same_file(&long_file_name[0], &next_file_name[0])) {
                        ...
                        if (dir_entry->file_attrib == ATTR_DIRECTORY) {
                            return search_directory_contents(first_cluster, &next_dir_name[0],
                                                             next_index + next_length + 1);
                        }
                        return first_cluster;
                    }
                    index += sizeof(struct dir_sfn_entry);
                    ...
                    uint32_t first_cluster =
                        ((uint32_t)(dir_entry->first_cluster_high << 16 | dir_entry->first_cluster_low)) & 0x0fffffff;
    ...
    if (LFdir) {                                             // Check the directory pointer valid
        for (j = 0; ((j < 5) && (lfn_done == false)); j++) { // For each of the first 5 characters
            ...
            if (utf == 0)
                ...
                LFNtext[LFN_blockcount++] =
                    (char) ascii_for_utf_16(utf); // Otherwise narrow to ascii and place in buffer
        }
...
void getFatEntrySectorAndOffset(uint32_t cluster_number, uint32_t* fat_sector_num, uint32_t* fat_entry_offset) {
    ...
    *fat_sector_num = current_sd_partition.unusedSectors + current_sd_partition.reservedSectorCount +
                      (fat_offset / current_sd_partition.bytesPerSector);
