...
void mutex_acquire(mutex_t* lock) {
    if (try_mutext_acquire(lock) == 0) {
        enqueue(lock->wait_queue, &current);
        block_this_task();
        printf("blocking pid: %d current: %x \n", current->pid, current);
    }
}

void mutex_release(mutex_t* lock) {
    lock->lock = 0;
    if (queue_size(lock->wait_queue) != 0) {
        task_struct_t* task;
        dequeue(lock->wait_queue, (void*) &task);
        wake_task(task);
        printf("Unblocking pid: %d current: %x \n", task->pid, task);
    }
}
