...
static uint32_t find_contiguous_frames_in_ranges(uint32_t start_frame_idx, uint32_t end_frame_idx,
                                                 uint32_t num_of_frames, uint32_t alignment);
...
static uint32_t find_free_frame() {
    ...
    if (free_frame_address == 0 && current_state.available_frames != 0) {
        ...
        free_frame_address = find_free_frame_in_rage(current_state.kernel_usage + current_state.frame_manager_usage,
...
static uint32_t find_contiguous_frames(uint32_t num_of_frames, uint32_t alignment) {
    uint32_t frame_address =
        find_contiguous_frames_in_ranges(current_state.next_idx, current_state.total_frames, num_of_frames, alignment);
    if (frame_address == 0 && current_state.available_frames >= num_of_frames) {
        // there might be a chance that we get contiguous allocation.
        frame_address = find_contiguous_frames_in_ranges(current_state.kernel_usage + current_state.frame_manager_usage,
                                                         current_state.next_idx - 1, num_of_frames, alignment);
    }
