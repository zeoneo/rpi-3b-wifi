...
static void
addscan(Block *bp, uint8_t *p, int len)
{
    char bssid[24];
    char *auth, *auth2;
    uint8_t *t, *et;
    ...
    bp->wp = (uint8_t*)seprint((char*)bp->wp, (char*)bp->lim, "ssid=%.*s%s;signal=%d;noise=%d;chan=%d", 
    ...
    bp->wp = (uint8_t*)seprint((char*)bp->wp, (char*)bp->lim,
        "%s%s\n", auth, auth2);
}

static void
wlscanresult(Ether *edev, uint8_t *p, uint32_t len)
{
    Ctlr *ctlr;
    Netfile **ep, *f, **fp;
    Block *bp;
...
void etherbcmattach(Ether *edev)
{
    ...
    printf("rproc : %x \n", (uint32_t)&rproc);
    printf("lproc : %x \n", (uint32_t)&lproc);
    copy_process((uint32_t)&rproc, (uint32_t)edev); //"wifireader", 
    copy_process((uint32_t)&lproc, (uint32_t)edev); //wifitimer
}
...
static int
wpaparsekey(WKey *key, uint64_t *ivp, char *a)
{
    int len;
    char *e;
...
static void
wlwpakey(Ctlr *ctl, int id, uint64_t iv, uint8_t *ea)
{
    uint8_t params[164];
    uint8_t *p;
    int pairwise;
...
static void
etherbcmscan(void *a, uint32_t secs)
{
    Ether* edev;
    Ctlr* ctlr;
