...
extern volatile struct HostGlobalRegs {
    volatile struct {
        volatile enum { Clock30_60MHz, Clock48MHz, Clock6MHz } ClockRate : 2; // @0
...
Result HcdPrepareChannel(struct UsbDevice* device, uint8_t channel, uint32_t length, enum PacketId type,
                         struct UsbPipeAddress* pipe);

Result HcdChannelSendWait(struct UsbDevice* device, struct UsbPipeAddress* pipe, uint8_t channel, void* buffer,
                          uint32_t bufferLength, struct UsbDeviceRequest* request, enum PacketId packetId);

Result HcdSumbitControlMessage(struct UsbDevice* device, struct UsbPipeAddress pipe, void* buffer,
                               uint32_t bufferLength, struct UsbDeviceRequest* request);

Result HcdChannelSendWaitOne(struct UsbDevice* device, struct UsbPipeAddress* pipe, uint8_t channel, void* buffer,
                             uint32_t bufferLength, uint32_t bufferOffset, struct UsbDeviceRequest* request);

Result HcdInterruptPoll(struct UsbDevice* device, struct UsbPipeAddress pipe, void* buffer, uint32_t bufferLength,
